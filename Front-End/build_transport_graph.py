...
def build_graph():
    ...
    for u in dead_ends:
        ...
        # [User Request] Skip "solo shipping points" at the dateline (-179 / +179)
        # We handle Dateline connectivity in the Frontend now.
        if abs(u_lon) > 175.0:
            continue
    ...
    # [User Request] "do not add any edges" for solo points at -179/+179.
    # Logic disabled because we now handle this dynamically in the Frontend (GraphEngine/GraphIndex).
    logging.info("Skipping backend Dateline connections (Handled in Frontend).")
    dateline_edges_count = 0
    # The block below is effectively disabled by not running it
